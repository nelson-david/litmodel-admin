...
const ScoutedModelCard: React.FC<ScoutedModelCardProps> = ({ model, reloadModels }) => {
    const [currentImageIndex, setCurrentImageIndex] = useState(0);
    const [deleteModalOpen, setDeleteModalOpen] = useState(false);
    const [processingDelete, setProcessingDelete] = useState(false);

    const images = model.polaroid && model.polaroid.length > 0 ? model.polaroid : ["https://placehold.co/400x500"];
    ...
    const isNew = new Date(model.dateAdded).getTime() > Date.now() - 7 * 24 * 60 * 60 * 1000;
    ...
    return (
        <div className="neo-box p-3 bg-white flex flex-col h-full relative group">
            ...
            <div className="grow flex flex-col">
                ...
                <div className="mb-4">
                    <p className="font-bold text-[10px] uppercase tracking-widest text-gray-400 mb-2">Measurements</p>
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                        <div className="flex justify-between border-b border-gray-100 pb-1">
                            <span className="font-bold text-gray-500">Height</span>
                            <span className="font-black">{model.measurement?.height || "-"}</span>
                        </div>
                         <div className="flex justify-between border-b border-gray-100 pb-1">
                            <span className="font-bold text-gray-500">Hair</span>
                            <span className="font-black">{model.measurement?.hairColor || "-"}</span>
                        </div>
                        <div className="flex justify-between border-b border-gray-100 pb-1">
                            <span className="font-bold text-gray-500">Chest</span>
                            <span className="font-black">{model.measurement?.chestBust || "-"}</span>
                        </div>
                        <div className="flex justify-between border-b border-gray-100 pb-1">
                            <span className="font-bold text-gray-500">Waist</span>
                            <span className="font-black">{model.measurement?.waist || "-"}</span>
                        </div>
                        <div className="flex justify-between border-b border-gray-100 pb-1">
                            <span className="font-bold text-gray-500">Hips</span>
                            <span className="font-black">{model.measurement?.hips || "-"}</span>
                        </div>
                        <div className="flex justify-between border-b border-gray-100 pb-1">
                            <span className="font-bold text-gray-500">Shoe</span>
                            <span className="font-black">{model.measurement?.shoe || "-"}</span>
                        </div>
                    </div>
                </div>
                ...
                {/* Contact Info */}
                <div className="mb-4">
                    <p className="font-bold text-[10px] uppercase tracking-widest text-gray-400 mb-2">Contact</p>
                    <div className="space-y-2 text-xs">
                        {model.instagramHandle && (
                            <div className="flex items-center gap-2">
                                <span className="font-bold text-gray-500 w-16 uppercase text-[10px]">Instagram</span>
                                <a 
                        ...
                        {model.phoneNumber && (
                            <div className="flex items-center gap-2">
                                <span className="font-bold text-gray-500 w-16 uppercase text-[10px]">Phone</span>
                                <a 
                         ...
                         {model.residentialAddress && (
                            <div className="flex items-start gap-2">
                                <span className="font-bold text-gray-500 w-16 uppercase text-[10px] shrink-0">Address</span>
                                <span className="font-medium leading-tight">{model.residentialAddress}</span>
                            </div>
                        )}
                    </div>
                </div>
