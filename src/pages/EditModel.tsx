...
const EditModel = ({
	...
	useEffect(() => {
		if (!loading && models.length > 0) {
			const currentModel: any = models.find((model: any) => model._id === id);
			console.log("CUURRENT: ", currentModel);
			if (currentModel) {
				setModelName(currentModel.fullName);
				setGender(currentModel.gender);
				setShortDescription(currentModel.shortDescription);
				setMeasurement(currentModel.measurement);
				setPortfolio(currentModel.portfolio);
				setNewFace(currentModel.newFace);
				setTopModel(currentModel.topModel);
				setModelImages(currentModel.polaroid || []);
				setPortfolioImages(currentModel.portfolio?.images || []);
			}
		}
	}, [id, loading, models]);

	const handleUploadStart = () => setUploadCounter((prev) => prev + 1);
	const handleUploadEnd = () =>
		setUploadCounter((prev) => Math.max(0, prev - 1));

	const updateModel = (e: any) => {
		e.preventDefault();
		if (uploadCounter > 0) return; // double safety
		setProcessing(true);
	...
	return (
		<PageLayout>
			...
			<div className="max-w-5xl p-4 sm:p-8 md:p-16 mb-20 mx-auto bg-white border-[1.5px] border-black shadow-neo-lg">
				<div className="flex justify-between items-center mb-8 border-b pb-6">
					<div className="flex flex-col gap-1">
						...
						<p className="text-gray-500 font-medium text-sm sm:text-base">
							Update model details and portfolio.
						</p>
					</div>
				...
				<form onSubmit={updateModel} className="space-y-16">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
							...
							<div>
								...
								<input
									...
									onChange={(e) =>
										setPortfolio({ ...portfolio, other: e.target.value })
									}
									required
								/>
							</div>
								...
								<div>
									...
									<div className="flex gap-6">
										<label className="flex items-center gap-2 cursor-pointer group">
											<div
												className={`w-5 h-5 border-2 border-black flex items-center justify-center transition-colors ${newFace ? "bg-black" : "bg-white group-hover:bg-gray-100"}`}
											>
										...
										<label className="flex items-center gap-2 cursor-pointer group">
											<div
												className={`w-5 h-5 border-2 border-black flex items-center justify-center transition-colors ${topModel ? "bg-black" : "bg-white group-hover:bg-gray-100"}`}
											>
								...
								<div>
									...
									<div className="flex gap-6">
										{["male", "female", "non-binary"].map((g) => (
											...
											>
												<div
													className={`w-5 h-5 rounded-full border-2 border-black flex items-center justify-center transition-colors ${gender === g ? "bg-black" : "bg-white group-hover:bg-gray-100"}`}
												>
													{gender === g && (
